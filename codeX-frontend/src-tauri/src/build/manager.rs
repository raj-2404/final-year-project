...
use super::process::BuildProcess;

#[derive(Clone)]
pub struct BuildManager {
    processes: Arc<Mutex<HashMap<String, BuildProcess>>>,
}
...
impl BuildManager {
    ...
    /// Checks if a binary or compiler exists in system PATH.
    ...
    /// Starts a local build or run process.
    pub fn start_process(
        ...
        let mut child = cmd
            .spawn()
            .map_err(|e| format!("Failed to spawn build/run process '{}': {}", trimmed_exe, e))?;
                ...
                loop {
                    match out.read(&mut buffer) {
                        ...
                        Ok(n) => {
                            ...
                            let _ = app.emit(&format!("build-stdout-{}", p_id), text);
                        }
                        Err(_) => break,
                    }
                }
            ...
            thread::spawn(move || {
                ...
                loop {
                    match err.read(&mut buffer) {
                        ...
                        Ok(n) => {
                            let text = String::from_utf8_lossy(&buffer[..n]).to_string();
                            let _ = app.emit(&format!("build-stderr-{}", p_id), text);
                        }
                        Err(_) => break,
                    }
                }
    ...
    pub fn write_to_process(&self, id: &str, data: &str) -> Result<(), String> {
        ...
        if let Some(proc) = map.get(id) {
            ...
            if let Some(ref mut stdin) = *stdin_guard {
                ...
                stdin.flush().map_err(|e| format!("Failed to flush process stdin: {}", e))?;
                Ok(())
            } else {
    ...
    /// Gracefully stops a process (SIGTERM on Unix, kill on Windows).
    ...
    /// Terminates all running build/run processes on app shutdown.
